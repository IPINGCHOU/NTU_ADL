...
LINEAR_LEARNING_RATE = 5e-05
EPOCH = 20
THRESHOLD = 0.5

# import sys
# test_input = sys.argv[1]
# pred_output = sys.argv[2]
...
def preprocessing_dataset(data_paragraphs, flag, tokenizer):
    ...
    #TODO flag, (just for showing mode, no implement)
    print('Now mode: ' + str(flag))
...
for i, (qcs, segments, masks, ids) in trange:
    ...
    for i, (start, end, seg) in enumerate(zip(ans_start, ans_end, segments)):
        seg = seg[1:].cpu()
        q_len = np.where(seg==1)[0][0]
        _, start = start[seg>0].data.topk(1, dim = 0) + q_len + 1
        _, end = end[seg>0].data.topk(1, dim = 0) + q_len
...
with open('model_pred_7.json', 'w') as fp:
